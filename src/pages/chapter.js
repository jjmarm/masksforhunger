...
const ChapterPage = ({props}) => {
  const data = useStaticQuery(graphql`
    query {
      chapter: markdownRemark(frontmatter: {title: {eq: "Boston"}}) {
        frontmatter {
          title
          about
          backgroundColor
  ...
  const BackgroundImageStack = [`linear-gradient(to bottom, #${chapter.colorOne}88, rgba(255, 0, 0, 0) 50%)`, chapter.mainImage.childImageSharp.fluid]
  return (
    <Layout>
